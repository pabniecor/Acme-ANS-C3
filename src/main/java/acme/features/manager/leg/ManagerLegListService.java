...
public class ManagerLegListService extends AbstractGuiService<Manager, Leg> {
	...
	public void unbind(final Leg leg) {
		...
		super.addPayload(dataset, leg, "aircraft.model", "departureAirport.city", "arrivalAirport.city");
	...
	public void unbind(final Collection<Leg> legs) {
		...
		masterId = super.getRequest().getData("masterId", int.class);
		flight = this.repository.findFlightById(masterId);
		showCreate = super.getRequest().getPrincipal().hasRealm(flight.getManager()) && flight.getDraftMode();
